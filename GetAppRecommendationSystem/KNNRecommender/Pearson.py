...
def MLPearson(activityUrl,rankingsUrl):
	activity_names = pd.read_csv(activityUrl, sep=',', error_bad_lines=False, encoding="latin-1")
	activity_names.columns = ['activity_id', 'name', 'sub_id']
	ratings_data = pd.read_csv(rankingsUrl, sep=',', error_bad_lines=False, encoding="latin-1")
	ratings_data.columns = ['user_id', 'activity_id', 'rank', 'created_at']
	print ("*********ratings_data: *********")
	print(ratings_data)
	print ("*********/ratings_data: *********")
	print("\n")
	...
	football_rating = user_activity_rating['presentation']  
